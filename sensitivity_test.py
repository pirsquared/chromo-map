...
for base in base_colors:
    ...
    base_obj = Color(base)
    print(f'{base} ({base_obj.hsv[0]:3.0f}°) → {result.hex} (contrast: {result.contrast_ratio(target):.2f})')
...
for target_str in targets:
    ...
    result = find_maximal_contrast_binary_search(base, target)
    print(f'{base} vs {target_str} → {result.hex} (contrast: {result.contrast_ratio(target):.2f})')
